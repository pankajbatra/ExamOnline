...
public class AddQuestion1 extends HttpServlet{
	...
	throws ServletException,IOException{
		...
		String ad_user=null,file_name=null,ans_correct=null,table_name=null,table_statement=null,table_question=null,table_code=null,ans_ch_1=null,ans_ch_2=null,ans_ch_3=null,ans_ch_4=null;
		...
		try{
			...
			ans_correct = multi.getParameter("ans_ch_correct");
			...
			if(table_image==1){
				...
				file_name="image"+exten;										
				ans_ch_correct = Integer.parseInt(ans_correct);
				rs = stmt.executeQuery("select MAX(id) from "+table_name+"");
				if(rs.next()){
				id=rs.getInt("MAX(id)");
				if(rs.wasNull())id=0;
				}
				...
				if(rs.next()){
					no_qus=rs.getInt("COUNT(*)");
				}
				PreparedStatement pstmt = con.prepareStatement("insert into "+table_name+" values(?,?,?,?,?,?,?,?,?,?)");
				pstmt.clearParameters();
				pstmt.setInt(1,id);
				pstmt.setString(2,table_statement);
				pstmt.setString(3,table_question);
				pstmt.setInt(4,table_image);
				pstmt.setString(5,table_code);
				pstmt.setString(6,ans_ch_1);
				pstmt.setString(7,ans_ch_2);
				pstmt.setString(8,ans_ch_3);
				pstmt.setString(9,ans_ch_4);
				pstmt.setInt(10,ans_ch_correct);
				pstmt.executeUpdate();
				pstmt.close();
        	...
        	out.println("<tr>");
        	out.println("<td width='100%'><font face='Verdana' color='#FFFFFF'><small><strong>Question Added Successfully...</strong></small></font></td>");
        	out.println("</tr>");
        	out.println("</table>");
        	out.println("</div><div align='right'>");
        	...
        	out.println("</strong></font><font face='Verdana' color='#6767B4'>The question has been successfully added to the test.</font></small></td>");
        	out.println("</tr>");
        	out.println("<tr>");
        	out.println("<td width='100%' height='30'><p align='center'><small><font face='Verdana' color='#6767B4'>The total number of questions in this subject is :- </font><font face='Verdana' color='#0000FF'><strong>"+(no_qus+1)+"</strong></font></small></td>");
        	out.println("</tr>");
        	out.println("<tr>");
        	out.println("<td width='100%' height='30'><p align='center'><font face='Verdana' color='#6767B4'><small>You may now add more questions to the test using the link below.</small></font></td>");
        	out.println("</tr>");
    	    out.println("<tr>");
	        out.println("<td width='100%' height='30'><p align='center'><a href='/servlet/AddQuestion?table_name="+table_name+"' target='_self'><font face='Verdana' color='#6767B4'><small>Add Questions</small></font></a></td>");
	        out.println("</tr>");
    	    out.println("</table>");
        	out.println("</td>");
        	out.println("</tr>");
	...
	public String getServletInfo(){
		return "Servlet to Add Question in a test table";
	}	
}