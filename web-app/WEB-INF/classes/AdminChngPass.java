...
public class AdminChngPass extends HttpServlet
{
	...
	throws ServletException,IOException
	{
		...
		String aduser,adpasswd,newpasswd,vnewpasswd;
		...
		try
		{
			...
			if(adpasswd==null||newpasswd==null||vnewpasswd==null){
			...
			if(adpasswd.equals("")||newpasswd.equals("")||vnewpasswd.equals("")){
				out.println("<H2>Some of The fields were left blank</H2>");
				out.println("<a href='javascript:history.go(-1)'>Click Here</a> to go back & Try Again");
				return;
			}
			...
			while(rs.next())
			{
				pass=rs.getString("adpasswd");
			}
			rs.close();
			if(!adpasswd.equals(pass)){
				out.println("<H2>Old Password is Wrong</H2>");
				out.println("<a href='javascript:history.go(-1)'>Click Here</a> to go back & Try Again");
				return;
			}
			int update=stmt.executeUpdate("update admin set adpasswd='"+newpasswd+"' where aduser='"+aduser+"' AND adpasswd='"+adpasswd+"'");
			if(update==0){
            ...
            session.invalidate();
			out.println("<tr>");
            out.println("<td width='100%' height='30'><p align='center'><font face='Verdana' color='#8000FF'><small><a href='../examonline/admin.html' target='_top'>Click Here</a> to Login Again!</small></font></td>");
            out.println("</tr>");
        	out.println("</table>");
	        out.println("</td>");
            out.println("</tr>");
    ...
    public String getServletInfo(){
		return "This servlet Changes Admin Password";
	}	
}		
